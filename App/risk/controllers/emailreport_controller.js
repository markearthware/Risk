        ...
        },
    {
        ...
        loadData: function () {
            ...
            $.when(taskDef).done(function (taskRes) {
                self.task = taskRes;
                var view = $.View('//risk/views/email/init.ejs');
            ...
            if ($('#EmailReportForm').valid()) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var sentTask = { id: self.task.id, Name: self.task.Name, DateStarted: self.task.DateStarted, DateFinished: new Date().getTime(), Sent: 1 };
